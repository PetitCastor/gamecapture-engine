...
sink.WriteLine("=== OCRX — Capture Engine ===");
...
// A bound pipe proves the new engine completed its startup gate. Only now retire the old product:
// its supported silent uninstaller must succeed before the literal v1 data directory is removed.
await LegacyMigration.TryCompleteAsync(sink);
