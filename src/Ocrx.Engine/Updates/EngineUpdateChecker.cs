...
{
    // The public binary-only repository carries releases.win.json and its payloads; engine source
    // remains private. OcrxEngine is the permanent v2 update-feed identity.
    private const string RepoUrl = "https://github.com/PetitCastor/ocrx-releases";
    ...
    public static async Task CheckAsync(ConsoleSink sink)
    {
        ...
        if (!TryAsk(
            $"OCRX v{update.TargetFullRelease.Version} is available (installed v{manager.CurrentVersion}).\n\nInstall now? The engine will restart.",
            "OCRX update available",
            out var accepted))
        {
        ...
        catch (Exception ex)
        {
            ...
            TryNotify(
                $"Could not install the update:\n{ex.Message}\n\nOCRX will continue on the current version.",
                "OCRX update failed");
        }
    }
