...
public class CaptureGrpcServiceTrackTests
{
    ...
    public async Task Track_WhenThePumpFaultsOnAnUnsupportedHello_SurfacesTheRefusalNotATimeout()
    {
        ...
        Assert.Equal(ProtocolVersion.Min.ToString(), ex.Trailers.GetValue("gamecapture-protocol-min"));
        Assert.Equal(ProtocolVersion.Current.ToString(), ex.Trailers.GetValue("gamecapture-protocol-max"));
