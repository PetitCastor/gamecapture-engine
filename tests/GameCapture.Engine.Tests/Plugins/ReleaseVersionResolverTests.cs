...
public class ReleaseVersionResolverTests
{
    ...
    {
        Assert.True(Extract(
            "https://github.com/PetitCastor/gamecapture-plugins/releases/download/v1.0.4/MissionPlugin-win-x64.zip",
            out var tag));
    ...
    {
        Assert.True(Extract(
            "https://github.com/PetitCastor/gamecapture-plugins/releases/download/v2.0.0-rc.1/A.zip",
            out var tag));
    ...
    public void TrailingDownloadSegment_HasNothingToRead()
        => Assert.False(Extract("https://github.com/PetitCastor/gamecapture-plugins/releases/download", out _));

    [Fact]
    public void EscapedTag_IsUnescaped()
    {
        Assert.True(Extract("https://github.com/PetitCastor/gamecapture-plugins/releases/download/v1%2E2%2E3/A.zip", out var tag));
