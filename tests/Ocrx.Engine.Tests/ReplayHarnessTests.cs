...
/// <summary>
/// <see cref="ReplayHarness"/> against a real, separately spawned <c>Ocrx.Engine.exe</c> — the
...
public class ReplayHarnessTests(ITestOutputHelper output)
{
    ...
    public async Task WhenTheEngineNeverComesUp_ThrowsTimeoutExceptionNamingTheFailure()
    {
        ...
        // Ocrx.Engine/Program.cs), so its stderr — captured in the ring buffer — says exactly why,
    ...
    public async Task AfterATimedOutRun_NoOrphanedEngineProcessRemains()
    {
        var enginePath = EngineLocator.Resolve();
        var missingCorpus = Path.Combine(Path.GetTempPath(), $"sc-replay-missing-{Guid.NewGuid():N}");
        var before = Process.GetProcessesByName("Ocrx.Engine").Length;
        ...
        while (Process.GetProcessesByName("Ocrx.Engine").Length > before)
        {
            cts.Token.ThrowIfCancellationRequested();
            await Task.Delay(50, cts.Token);
        }

        Assert.Equal(before, Process.GetProcessesByName("Ocrx.Engine").Length);
    }
}
