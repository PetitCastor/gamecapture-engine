...
public class ProtocolHandshakeTests
{
    ...
    public async Task Hello_v1_is_acknowledged_with_the_negotiated_version()
    {
        ...
        using var channel = TestNamedPipeChannel.Create(engine.PipeName);
    ...
    public async Task Hello_v999_faults_with_supported_protocol_range()
    {
        ...
        Assert.Equal(ProtocolVersion.Min.ToString(), exception.Trailers.GetValue("ocrx-protocol-min"));
        Assert.Equal(ProtocolVersion.Current.ToString(), exception.Trailers.GetValue("ocrx-protocol-max"));
    }
    ...
    /// registered client â€” registration happens when the Track call opens, before the client's
    /// Hello has been read â€” and the outbound channel drops its OLDEST entry on overflow. An ack
