...
What `Ocrx.Engine` offers over the wire, one section per RPC in `protos/capture.proto`. This is
...
`src/Ocrx.Engine/Grpc/CaptureGrpcService.cs:51-170`).
...
`src/Ocrx.Engine/ScanLoop.cs`). A failing ROI never removes another ROI's result, for
...
`RoiMode` enum at `protos/capture.proto:50-54`; SDK-side `RoiKind`, `src/Ocrx.Sdk/RoiKind.cs`):
...
`src/Ocrx.Contracts/RoiScaler.cs:12-13`). **The engine does all scaling** — `RoiScaler.ToFrame`
maps a reference rect to the actual frame pixels for whatever resolution is being captured
(`ScanLoop.ReadOneAsync`, `src/Ocrx.Engine/ScanLoop.cs`), and the frame-space rect actually
...
(`src/Ocrx.Contracts/WireLimits.cs:11,27-28`). Before capturing, the engine clamps the requested
scale so the upscaled crop never exceeds `OcrEngine.MaxImageDimension` (a Windows OCR limit) on
its longest side (`OcrPipeline.EffectiveScale`, `src/Ocrx.Engine/Processing/OcrPipeline.cs:120-126`).
...
(`ClientSubscription.OutboundCapacity`, `src/Ocrx.Engine/ClientSubscription.cs:14-18` — roughly 2 s
...
On the SDK side, `FrameSequenceTracker` (`src/Ocrx.Sdk/Plugin/FrameSequenceTracker.cs`) watches
`FrameSeq` across ticks and `OcrxPluginHost`'s dispatcher raises `SessionEvent.TicksDropped(Gap)`
when it detects a frame-sequence gap (`src/Ocrx.Sdk/Plugin/SessionEvent.cs:28-39`) — a normal,
...
(`FrameSequenceTracker.TryObserve`, `src/Ocrx.Sdk/Plugin/FrameSequenceTracker.cs:31-42`).
...
of that bit (`src/Ocrx.Sdk/TickData.cs:67`); the plugin host dispatches a manual tick to
`IOcrxPlugin.OnManualTickAsync` instead of `OnTickAsync` (`TickDispatcher.DispatchAsync`,
`src/Ocrx.Sdk/Plugin/TickDispatcher.cs:78-79`), which a plugin overrides when the hotkey means
something different from its normal capture (`IOcrxPlugin.cs:49-55`).
...
negotiation, and the rejection path (`FAILED_PRECONDITION` with `ocrx-protocol-min/max`
...
`src/Ocrx.Engine/Grpc/CaptureGrpcService.cs:176-197`).
...
- **Calibration workflow**: point a throwaway `RoiSpec` at a candidate rectangle, call `ReadRoi`
  ...
  (`src/Ocrx.Sdk/CaptureClient.cs:236`).
...
`src/Ocrx.Engine/Grpc/CaptureGrpcService.cs:203-237`).
...
- **Output dir ownership**: always `EngineConfig.OutputDir` (`src/Ocrx.Engine/Configuration/EngineConfig.cs:19`)
  — the engine's own config, never anything the client supplies directly. Relative paths in
  `%LOCALAPPDATA%\OCRX\engine-config.json` resolves relative paths against the config file's own directory (`EngineConfig.Load`,
...
`src/Ocrx.Engine/Grpc/CaptureGrpcService.cs:239-250`; assembled from `EngineStatus.Snapshot()`,
`src/Ocrx.Engine/EngineStatus.cs:68-86`).
...
| `connected_clients` | Names of every connection currently open on the engine, ordinal-sorted — not only ones that have subscribed. A client is listed the moment its `Track` call registers, as `"?"` until its `Hello` names it (`SubscriptionRegistry.Register`, `src/Ocrx.Engine/SubscriptionRegistry.cs:26-36`); a bare `GetStatus`/`WaitForEngineAsync` call opens no `Track` stream and does not appear. |
| `min_supported_protocol` / `max_supported_protocol` | The `[Min, Current]` protocol-version range this engine build accepts — `Ocrx.Contracts.ProtocolVersion`; see the version-policy section of `docs/PROTOCOL.md`. |
| `scan_interval_ms` | The cadence the scan loop *actually* runs at, after its own minimum clamp — not the raw config value. `0` means an engine older than this field; a client falls back to `EngineDefaults.DefaultScanInterval` (500 ms, `src/Ocrx.Sdk/EngineDefaults.cs:39`). |
...
(`src/Ocrx.Sdk/CaptureClient.cs:70`, discussed in the Transport section of `docs/PROTOCOL.md`).

## Replay mode

`--replay <dir>` feeds a directory of full-frame PNGs through the exact same `ScanLoop` production
uses instead of live WGC capture (`ReplayFrameSource`, `src/Ocrx.Engine/Capture/ReplayFrameSource.cs`;
...
- Frames are enumerated once at construction, sorted by filename in **ordinal** order
  (`ReplayFrameSource.EnumerateCorpus`, `src/Ocrx.Engine/Capture/ReplayFrameSource.cs:50-51`) — not
...
- The scan loop does not start consuming the corpus until **at least one client has sent a
  ...
  `src/Ocrx.Engine/SubscriptionRegistry.cs:48-57`) — otherwise a corpus could be consumed into the
...
`VideoFrameSource` (`src/Ocrx.Engine/Capture/VideoFrameSource.cs`; wired in
...
`src/Ocrx.Engine/Operations/FrameDumpService.cs`). Cannot be combined with `--replay`
...
(`src/Ocrx.Engine/Hosting/HotkeyListener.cs:6-11`) — `RegisterHotKey` is not used because a game
...
| Budget | Value | Source |
| --- | --- | --- |
| `ROI_MODE_PIXELS` payload cap | 256 KiB (a 256x256 BGRA patch) | `WireLimits.MaxPixelBytes`, `src/Ocrx.Contracts/WireLimits.cs:20` (re-exported as `EngineDefaults.MaxPixelBytes`, `src/Ocrx.Sdk/EngineDefaults.cs:46`). Checked against **frame-space** bounds, i.e. after `RoiScaler.ToFrame` (`ScanLoop.ReadOneAsync`) — a probe sized to fit at 2560x1440 can still exceed the cap on a higher-resolution capture. |
| OCR upscale clamp | Crop's longest side capped at `OcrEngine.MaxImageDimension` (Windows OCR API limit) | `OcrPipeline.EffectiveScale`, `src/Ocrx.Engine/Processing/OcrPipeline.cs:120-126` |
| Minimum scan interval | 100 ms | `ScanLoop.MinScanInterval`, `src/Ocrx.Engine/ScanLoop.cs` |
| Default scan interval | 500 ms (a stock, unconfigured engine) | `EngineConfig.ScanIntervalMs` default, `src/Ocrx.Engine/Configuration/EngineConfig.cs:32`; re-exported as `EngineDefaults.DefaultScanInterval`, `src/Ocrx.Sdk/EngineDefaults.cs:39` |
| Reference ROI space | 2560x1440 | `RoiScaler.ReferenceWidth/Height`, `src/Ocrx.Contracts/RoiScaler.cs:12-13` |
| Pixel byte order | BGRA | `EngineDefaults.PixelChannelOrder`, `src/Ocrx.Sdk/EngineDefaults.cs:54`; produced by `PixelStrip.CaptureAsync`, `src/Ocrx.Engine/Processing/PixelSampler.cs:41-58` |
| Outbound tick channel depth | 4 ticks (~2 s at the default cadence) | `ClientSubscription.OutboundCapacity`, `src/Ocrx.Engine/ClientSubscription.cs:14-18` |
...
- [`docs/PLUGIN-AUTHORING.md`](PLUGIN-AUTHORING.md) — writing an `IOcrxPlugin` against this
  catalog.
